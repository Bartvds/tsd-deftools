...
module tsdimport {
	...
	export class Encode implements DataEncoder {
		...
		encode(header:HeaderData):any {
			var ret = {
				"name": header.name + (header.submodule ? ' (' + header.submodule + ')' : ''),
				"description": header.description,
				"notes": [<any>'generated by tst-deftools', header.reposUrl, new Date().toUTCString()],
				"versions": [
					{
